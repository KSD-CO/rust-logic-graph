...
use crate::distributed::context::{ContextSnapshot, DistributedContext};
...
impl VersionedContext {
    ...
    pub fn merge_with_resolution(&mut self, other: &DistributedContext) -> Result<()> {
        match self.resolution_strategy {
            ConflictResolution::LastWriteWins => self.merge_last_write_wins(other),
            ConflictResolution::HigherVersionWins => self.merge_higher_version_wins(other),
            ConflictResolution::FailOnConflict => self.merge_fail_on_conflict(other),
            ConflictResolution::MergeAll => self.merge_all(other),
        }
    }
