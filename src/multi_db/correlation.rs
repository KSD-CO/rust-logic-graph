...
impl QueryCorrelator {
    ...
    pub fn join(
        ...
        info!("🔗 Query Correlator: Joining datasets on {}.{} = {}.{} ({:?})",
        ...
        debug!("Left dataset: {} rows, Right dataset: {} rows", left_arr.len(), right_arr.len());
        ...
        for right_row in right_arr {
            if let Some(key_value) = right_row.get(right_key) {
                ...
                right_index.entry(key_str).or_insert_with(Vec::new).push(right_row);
            }
        }
...
mod tests {
    ...
    fn test_inner_join() {
        ...
        let result = correlator.join(&users, &orders, "user_id", "user_id", JoinStrategy::Inner).unwrap();
    ...
    fn test_left_join() {
        ...
        let result = correlator.join(&users, &orders, "user_id", "user_id", JoinStrategy::Left).unwrap();
        let arr = result.as_array().unwrap();
    ...
    fn test_with_prefixes() {
        ...
        let result = correlator.join(&users, &orders, "id", "user_id", JoinStrategy::Inner).unwrap();
        let arr = result.as_array().unwrap();
