...
use repository::PgUomRepository;
use grpc_handler::{uom::uom_service_server::UomServiceServer, UomGrpcService};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create repository
    let repository = Arc::new(PgUomRepository::new(pool));

    // Start gRPC server
    let grpc_repository = repository.as_ref().clone();
    let grpc_addr = server_config.grpc_addr().parse()?;
    let grpc_service = UomGrpcService::new(grpc_repository);
    let grpc_server = Server::builder()
        .add_service(UomServiceServer::new(grpc_service))
        .serve(grpc_addr);
