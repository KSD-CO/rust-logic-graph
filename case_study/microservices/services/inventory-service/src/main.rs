...
use repository::PgInventoryRepository;
use grpc_handler::{inventory::inventory_service_server::InventoryServiceServer, InventoryGrpcService};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create repository
    let repository = Arc::new(PgInventoryRepository::new(pool));

    // Start gRPC server
    let grpc_repository = repository.as_ref().clone();
    let grpc_addr = server_config.grpc_addr().parse()?;
    let grpc_service = InventoryGrpcService::new(grpc_repository);
