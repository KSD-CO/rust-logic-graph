...
use repository::PgOmsRepository;
use grpc_handler::{oms::oms_service_server::OmsServiceServer, OmsGrpcService};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create repository
    let repository = Arc::new(PgOmsRepository::new(pool));

    // Start gRPC server
    let grpc_repository = repository.as_ref().clone();
    let grpc_addr = server_config.grpc_addr().parse()?;
    let grpc_service = OmsGrpcService::new(grpc_repository);
    let grpc_server = Server::builder()
        .add_service(OmsServiceServer::new(grpc_service))
        .serve(grpc_addr);
