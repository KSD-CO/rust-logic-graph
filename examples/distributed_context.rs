...
use rust_logic_graph::distributed::{
    CacheStrategy, ConflictResolution, DistributedCache, DistributedContext, InMemoryStore,
    SharedContext, ThreeWayMerge, VersionedContext,
};
use serde_json::json;
use std::sync::Arc;
...
async fn example_state_sharing() -> anyhow::Result<()> {
    ...
    order_context
        .set("created_by", json!("order-service"))
        .await;
    ...
    println!(
        "  Transmitted {} bytes to Inventory Service",
        serialized.len()
    );
    ...
    println!(
        "  Cart Total: {}",
        inventory_context.get("cart_total").unwrap()
    );
    ...
    println!(
        "  Modified by: {}",
        inventory_context.metadata.modified_by.as_ref().unwrap()
    );
    ...
    println!(
        "  All keys: {:?}",
        payment_context.data.keys().collect::<Vec<_>>()
    );
...
async fn example_distributed_caching() -> anyhow::Result<()> {
    ...
    let batch_results = cache
        .get_many(&["batch-1".to_string(), "batch-2".to_string()])
        .await?;
...
fn example_conflict_resolution() -> anyhow::Result<()> {
    ...
    let mut vctx =
        VersionedContext::with_config("conflict-test", 5, ConflictResolution::LastWriteWins);
    ...
    let mut vctx2 =
        VersionedContext::with_config("version-test", 5, ConflictResolution::HigherVersionWins);
    ...
    println!(
        "  Result: status = {}",
        vctx2.current.get("status").unwrap()
    );
    println!("  ✓ Higher version wins\n");

    // Strategy 3: Merge All
    println!("Strategy 3: Merge All");
    let mut vctx3 = VersionedContext::with_config("merge-test", 5, ConflictResolution::MergeAll);
    ...
    println!(
        "    local_key = {}",
        vctx3.current.get("local_key").unwrap()
    );
    println!(
        "    remote_key = {}",
        vctx3.current.get("remote_key").unwrap()
    );
...
fn example_three_way_merge() -> anyhow::Result<()> {
    ...
    println!(
        ...
        merged.get("description").unwrap()
    );
    println!(
        "  category: {} (from remote)",
        merged.get("category").unwrap()
    );
