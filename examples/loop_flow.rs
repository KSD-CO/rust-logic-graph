...
async fn main() -> anyhow::Result<()> {
    ...
    let foreach_loop: Box<dyn Node> = Box::new(LoopNode::new_foreach(
        "process_products",
        "products"
    ));

    let mut ctx = Context::new();
    ctx.set("products", json!([
    ...
    println!("While loop result: {}", serde_json::to_string_pretty(&result)?);
    println!();
    ...
    println!("Limited loop result: {}", serde_json::to_string_pretty(&result)?);
