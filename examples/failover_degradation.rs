use rust_logic_graph::fault_tolerance::{CircuitBreaker, CircuitConfig, FailoverManager, ServiceEndpoint};
use rust_logic_graph::distributed::InMemoryStore;
use rust_logic_graph::{Executor, Graph, GraphDef, NodeType, NodeConfig};
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    // Mock endpoints
    let endpoints = vec![
        ServiceEndpoint { name: "primary".into(), url: "http://127.0.0.1:3001/health".into() },
        ServiceEndpoint { name: "backup".into(), url: "http://127.0.0.1:3002/health".into() },
    ];

    let store = Arc::new(InMemoryStore::new());
    let cb = Arc::new(CircuitBreaker::new("payment-svc", Some(store.clone()), Some(CircuitConfig { failure_threshold: 2, recovery_timeout: std::time::Duration::from_secs(3), probe_interval: std::time::Duration::from_secs(1) })));
    ...
    exec.set_fallback_handler(|node_id, ctx| {
        ...
        ctx.data.insert(format!("{}_result", node_id), serde_json::Value::String("fallback".into()));
    ...
    exec.register_node(Box::new(rust_logic_graph::node::RuleNode::new("call_service", "false")));
